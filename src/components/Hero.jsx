      ...
      <div className="w-full pt-20 pb-4 md:pt-28 md:pb-16">
        <div className="w-full max-w-[1920px] mx-auto px-4 sm:px-6 md:px-12 relative z-10">
          <div className="grid md:grid-cols-2 gap-4 md:gap-12 items-center">
            ...
            >
              <div className="inline-block px-3 py-1.5 rounded-full glass border-cyan-500/30 text-cyan-400 text-xs md:text-sm font-medium mb-2 md:mb-6">
                Welcome to my portfolio
              </div>

              <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 dark:text-white mb-2 md:mb-4 tracking-tight leading-tight">
                Hi, I'm <br />
                <span className="text-gradient">Jeeshan</span>
              </h1>

              <h2 className="text-xs sm:text-base md:text-xl lg:text-2xl text-slate-700 dark:text-slate-300 font-medium mb-2 md:mb-6 min-h-[20px] md:min-h-[32px]">
                {text}<span className="animate-pulse text-cyan-400">|</span>
              </h2>

              <p className="text-slate-600 dark:text-slate-400 text-xs md:text-base max-w-md mb-4 md:mb-8 leading-relaxed mx-auto md:mx-0">
              ...
              <div className="flex flex-wrap gap-2 md:gap-3 justify-center md:justify-start">
                <Link to="projects" smooth={true} duration={500} offset={-70}>
                  <MagneticButton className="px-4 py-2 md:px-5 md:py-2.5 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-medium flex items-center gap-2 hover:shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all text-xs md:text-sm">
                    View Projects <ArrowRight size={14} className="md:w-[16px] md:h-[16px]" />
                  </MagneticButton>
                </Link>

                <Link to="contact" smooth={true} duration={500} offset={-70}>
                  <MagneticButton className="px-4 py-2 md:px-5 md:py-2.5 rounded-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 text-slate-700 dark:text-white font-medium hover:bg-slate-50 dark:hover:bg-white/10 transition-all flex items-center gap-2 shadow-sm text-xs md:text-sm">
                    Contact Me
                  </MagneticButton>
                </Link>

                <a href="/resume.html" target="_blank" rel="noreferrer">
                  <MagneticButton className="px-4 py-2 md:px-5 md:py-2.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-medium hover:bg-emerald-500 hover:text-white transition-all flex items-center gap-2 shadow-sm text-xs md:text-sm">
                    <Download size={13} className="md:w-[15px] md:h-[15px]" /> Download CV
                  </MagneticButton>
                </a>
              </div>

              {/* Social Links */}
              <div className="flex items-center gap-3 md:gap-4 mt-4 md:mt-8 justify-center md:justify-start flex-wrap">
                <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm font-medium">Follow Me:</p>
                ...
                <a href="http://instagram.com/jeeshan_abbasi18" target="_blank" rel="noreferrer" className="hover:scale-110 transition-transform">
                  <div className="w-[20px] h-[20px] md:w-[24px] md:h-[24px] rounded-[5px] flex items-center justify-center" style={{ background: 'radial-gradient(circle at 30% 107%, #fdf497 0%, #fdf497 5%, #fd5949 45%, #d6249f 60%, #285AEB 90%)' }}>
                    <FaInstagram size={12} className="text-white md:w-[15px] md:h-[15px]" />
                  </div>
                </a>
